...
class LedgerHistory {
   ...
   * format lets you pass in any valid aitdd ledger response data, regardless
...
class RequestManager {
  ...
  handleResponse(data: any) {
    ...
    if (data.status === 'error') {
      const error = new AitddError(data.error_message || data.error, data)
 ...
 * an active WebSocket connection to a AITDL node.
 */
export class Connection extends EventEmitter {
  ...
  private async _subscribeToLedger() {
    ...
    // If aitdd instance doesn't have validated ledgers, disconnect and then reject.
    if (_.isEmpty(data) || !data.ledger_index) {
      try {
        ...
        throw new AitddNotInitializedError('Aitdd not initialized')
      }
    }
    this._ledger.update(data)
  }
  ...
  connect(): Promise<void> {
    ...
    if (this._ws) {
      return Promise.reject(
        new AitdError('Websocket connection never cleaned up.', {
          state: this._state
        })
      )
    }
      ...
      this._onConnectionFailed(
        new ConnectionError(
          `Error: connect() timed out after ${this._config.connectionTimeout} ms. ` +
            `If your internet connection is working, the aitdd server may be blocked or inaccessible.`
        )
      )
  ...
  async reconnect() {
    ...
    // See: https://github.com/aitd/aitd-lib/pull/1101#issuecomment-565360423
    this.emit('reconnect')
    await this.disconnect()
    await this.connect()
  }
