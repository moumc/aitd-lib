...
function formatPrepareResponse(txJSON: any): Prepare {
  const instructions = {
    fee: common.dropsToAitd(txJSON.Fee),
    maxLedgerVersion:
 ...
 *
 *  See https://aitdl.org/transaction-malleability.html
 *
...
function prepareTransaction(
  ...
  function prepareFee(): Promise<void> {
    ...
    // Furthermore, txJSON.Fee is in drops while instructions.fee is in AITD, which would just add to
    ...
    if (instructions.fee !== undefined) {
      ...
      if (fee.isGreaterThan(api._maxFeeAITD)) {
        return Promise.reject(
          new ValidationError(
            `Fee of ${fee.toString(10)} AITD exceeds ` +
              `max of ${api._maxFeeAITD} AITD. To use this fee, increase ` +
              '`maxFeeAITD` in the AitdAPI constructor.'
          )
        )
      }
      newTxJSON.Fee = scaleValue(
        common.aitdToDrops(instructions.fee),
        multiplier
      )
      return Promise.resolve()
    }
    ...
    return api.getFee(cushion).then((fee) => {
      return api.connection.getFeeRef().then((feeRef) => {
        ...
        const feeDrops = common.aitdToDrops(fee)
        const maxFeeAITD = instructions.maxFee
          ? BigNumber.min(api._maxFeeAITD, instructions.maxFee)
          : api._maxFeeAITD
        const maxFeeDrops = common.aitdToDrops(maxFeeAITD)
