...
/**
 *  Check that a given transaction fee does not exceed maxFeeAITD (in drops).
 *
 *  See https://aitdl.org/aitdapi-reference.html#parameters
 *
 *  @param {AitdAPI} api A AitdAPI instance.
 *  @param {string} txFee The transaction fee in drops, encoded as a string.
 *
 *  @returns {void} This method does not return a value, but throws an error if the check fails.
 */
function checkFee(api: AitdAPI, txFee: string): void {
  const fee = new BigNumber(txFee)
  const maxFeeDrops = aitdToDrops(api._maxFeeAITD)
  if (fee.isGreaterThan(maxFeeDrops)) {
    throw new utils.common.errors.ValidationError(
      `"Fee" should not exceed "${maxFeeDrops}". ` +
        'To use a higher fee, set `maxFeeAITD` in the AitdAPI constructor.'
    )
  }
}
