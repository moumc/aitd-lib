...
function formatBalances(options, balances) {
  ...
  if (
    !(options.counterparty || (options.currency && options.currency !== 'AITD'))
  ) {
    const aitdBalance = {
      currency: 'AITD',
      value: balances.aitd
    }
    result.unshift(aitdBalance)
  }
...
function getBalances(
  ...
  // use tags independent of the AITD Ledger, comparing
  // with the AITD Ledger's balance as an accounting check.
  ...
  return Promise.all([
    getLedgerVersionHelper(
      ...
      utils.getAITDBalance(this.connection, address, ledgerVersion)
    ),
    this.getTrustlines(address, options)
  ]).then((results) =>
    formatBalances(options, {aitd: results[0], trustlines: results[1]})
  )
}
