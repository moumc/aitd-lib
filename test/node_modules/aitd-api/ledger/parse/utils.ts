...
import transactionParser from 'aitd-lib-transactionparser'
...
function adjustQualityForAITD(
  quality: string,
  takerGetsCurrency: string,
  takerPaysCurrency: string
) {
  // quality = takerPays.value/takerGets.value
  // using drops (1e-6 AITD) for AITD values
  const numeratorShift = takerPaysCurrency === 'AITD' ? -6 : 0
  const denominatorShift = takerGetsCurrency === 'AITD' ? -6 : 0
...
function parseTimestamp(aitdTime?: number | null): string | undefined {
  if (typeof aitdTime !== 'number') {
    return undefined
  }
  return common.aitdTimeToISO8601(aitdTime)
}
