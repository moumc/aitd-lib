...
export default <TestSuite>{
  'normal': async (api, address) => {
    ...
    await Promise.all([
      ...
      const orderbook = AitdAPI.formatBidsAndAsks(orderbookInfo, [
        ...directOffers,
        ...reverseOffers
      ])
  ...
  'with AITD': async (api, address) => {
    ...
    await Promise.all([
      ...
      assert.deepEqual(orderbook, responses.getOrderbook.withAITD)
    })
  },

  'sample AITD/JPY book has orders sorted correctly': async (api, address) => {
    const orderbookInfo = {
      base: {
    ...
    await Promise.all([
      api.request('book_offers', {
        ...
        taker_pays: AitdAPI.renameCounterpartyToIssuer(orderbookInfo.counter),
        ledger_index: 'validated',
        limit: 400, // must match `test/fixtures/aitdd/requests/1-taker_gets-AITD-taker_pays-JPY.json`
        taker: myAddress
      }),
      api.request('book_offers', {
        taker_gets: AitdAPI.renameCounterpartyToIssuer(orderbookInfo.counter),
        taker_pays: AitdAPI.renameCounterpartyToIssuer(orderbookInfo.base),
        ledger_index: 'validated',
        limit: 400, // must match `test/fixtures/aitdd/requests/2-taker_gets-JPY-taker_pays-AITD.json`
        taker: myAddress
      })
    ]).then(([directOfferResults, reverseOfferResults]) => {
      const directOffers = (directOfferResults
  ...
  'sample USD/AITD book has orders sorted correctly': async (api, address) => {
    const orderbookInfo = {
      counter: {currency: 'AITD'},
      base: {
        currency: 'USD',
        counterparty: 'rvYAfWj5gh67oV6fW32ZzP3Aw4Eubs59B'
      }
