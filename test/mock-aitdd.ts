...
import fixtures from './fixtures/aitdd'
import addresses from './fixtures/addresses.json'
import hashes from './fixtures/hashes.json'
import transactionsResponse from './fixtures/aitdd/account-tx'
import accountLinesResponse from './fixtures/aitdd/account-lines'
import accountObjectsResponse from './fixtures/aitdd/account-objects'
import fullLedger from './fixtures/aitdd/ledger-full-38129.json'
...
export function createMockAitdd(port) {
    ...
    if (request.account === addresses.ACCOUNT) {
      ...
      const response = Object.assign(
        {},
        {
          ...
          request:
            // This will be inaccurate, but that's OK because this is just a mock aitdd
            {
  ...
  mock.on('request_book_offers', function (request, conn) {
    if (request.taker_pays.issuer === 'rp8rJYTpodf8qbSCHVTNacf8nSW8mRakFw') {
      conn.send(createResponse(request, fixtures.book_offers.aitd_usd))
    } else if (
      request.taker_gets.issuer === 'rp8rJYTpodf8qbSCHVTNacf8nSW8mRakFw'
    ) {
      conn.send(createResponse(request, fixtures.book_offers.usd_aitd))
    } else if (
      ...
      const aitddDir = 'test/fixtures/aitdd'
      if (!requestsCache) {
        requestsCache = fs.readdirSync(aitddDir + '/requests')
      }
      for (var i = 0; i < requestsCache.length; i++) {
        const file = requestsCache[i]
        const json = fs.readFileSync(aitddDir + '/requests/' + file, 'utf8')
        ...
        if (JSON.stringify(requestWithoutId) === JSON.stringify(r)) {
          const responseFile =
            aitddDir + '/responses/' + file.split('.')[0] + '-res.json'
  ...
  mock.on('request_aitd_path_find', function (request, conn) {
    let response = null
    ...
    if (request.source_account === 'rB2NTuTTS3eNCsWxZYzJ4wqRqxNLZqA9Vx') {
      ...
      response = createResponse(request, {
        ...
        result: {
          ...
          destination_currencies: ['EUR', 'AITD'],
          full_reply: true,
          id: 2,
      ...
      response = createResponse(request, fixtures.path_find.AitdToAitd, {
