...
import {RecursiveData} from 'aitd-api/ledger/utils'
import {assertRejects, assertResultMatch} from './utils'
import addresses from './fixtures/addresses.json'
import responses from './fixtures/responses'
import ledgerClosed from './fixtures/aitdd/ledger-close-newer.json'
import setupAPI from './setup-api'

const {validate, schemaValidator, ledgerUtils} = AitdAPI._PRIVATE
...
describe('AitdAPI', function () {
  ...
  it('common utils - toAitddAmount', async () => {
    const amount = {issuer: 'is', currency: 'c', value: 'v'}
    assert.deepEqual(ledgerUtils.common.toAitddAmount(amount), {
      issuer: 'is',
      currency: 'c',
      value: 'v'
    })
  ...
  it('ledger utils - renameCounterpartyToIssuerInOrder', async () => {
    const order = {
      taker_gets: {counterparty: '1', currency: 'AITD'},
      taker_pays: {counterparty: '1', currency: 'AITD'}
    }
    const expected = {
      taker_gets: {issuer: '1', currency: 'AITD'},
      taker_pays: {issuer: '1', currency: 'AITD'}
    }
    assert.deepEqual(
