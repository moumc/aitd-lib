...
import {isValidAddress} from 'aitd-address-codec'
import {payTo, ledgerAccept} from './utils'
import {errors} from 'aitd-api/common'
import {isValidSecret} from 'aitd-api/common/utils'
...
function setup(this: any, server = 'wss://s1.aitd.com') {
  this.api = new AitdAPI({server})
...
function setupAccounts(testcase) {
  ...
  const promise = payTo(api, 'rMH4UxPrbuMa1spCBR98hLLyNJp4d8p4tM')
    ...
    .then(() => {
      return api
        .prepareSettings(masterAccount, {defaultAitd: true})
...
describe('integration tests', function () {
  ...
  it('payment', function () {
    const amount = {currency: 'AITD', value: '0.000001'}
  ...
  it('getOrderbook', function () {
    ...
    return this.api.getOrderbook(address, orderbook).then((book) => {
      ...
      assert.strictEqual(bid.specification.quantity.currency, 'AITD')
      ...
      assert.strictEqual(ask.specification.quantity.currency, 'AITD')
...
describe('integration tests - standalone aitdd', function () {
  const instructions = {maxLedgerVersionOffset: 10}
  this.timeout(TIMEOUT)
